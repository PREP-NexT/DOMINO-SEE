...
def plot_joint_degree(ax, degree, cmap=mpl.cm.YlOrRd):
    ...
    # Calculate moving average with 2-degree latitude window
    ...
    pxr.format(ylim=(newlat.min(), newlat.max()), xlim=(6000, 0),  # set xlim to fix axis direction
...
for nax in range(len(ax)):
    ...
    if nax == 0:
        ...
        cbar = ax[nax].colorbar(cs, location='bottom', orientation='horizontal', width=0.15, 
                                ticks=[150, 300, 500, 800, 1300, 2200, 3700, 6000, 10000],
